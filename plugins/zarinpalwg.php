...
$pluginData[zarinpalwg][author][name] = 'Freer & update to rest a.yousefi';
...
function gateway__zarinpalwg($data)
{
    ...
    else
    {
        ...
        $data[message] = '<font color="red">در اتصال به درگاه زرین‌پال مشکلی به وجود آمد٬ لطفا از درگاه سایر بانک‌ها استفاده نمایید.</font>'.$result['Status'].'<br /><a href="index.php" class="button">بازگشت</a>';
...
function callback__zarinpalwg($data)
{
    ...
    if ($_GET['Status'] == 'OK')
    {
        ...
        {
            if ($result['Status'] == 100)//-- موفقیت آمیز
            {
                //-- آماده کردن خروجی
                $output[status]		= 1;
            ...
            else
            {
                ...
                $output[status]	= 0;
                $output[message]= 'پرداخت توسط زرین‌پال تایید نشد‌.'.$result['Status'];
            }
        }
        else
        {
